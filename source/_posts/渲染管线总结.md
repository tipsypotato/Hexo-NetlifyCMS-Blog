...
![Untitled](images/pipeline-summary/Untitled.png)
...
- 光栅化阶段：
    ...
    **三角形设置&遍历**(不可编程)：输入为三角形的三个顶点，**三角形设置**的任务是得到三角形边界的表示，然后**遍历**会检查每个像素是否被三角形网格覆盖(Overlap)，若覆盖则生成一个片元，最简单的 Point sampling 采样像素点的中心，除此之外还有用于抗锯齿的**超采样(SSAA)**和**多重采样(MSAA)**，得到的片元包含了颜色、法线、纹理坐标、深度等属性，他们都是由三角形的顶点插值得到的，如果是透视投影，需要透视矫正插值得到正确的片元属性。（early-z 可以将深度测试提前到这里）
